...
const capabilities = [
  { cat: 'Discipline 01', title: 'UI / UX Design', desc: 'Interfaces that communicate precision at first sight and guide users with effortless intelligence. We design for emotion, then optimise for conversion.', list: ['Brand discovery & visual strategy', 'Wireframing & information architecture', 'High-fidelity visual design', 'Motion design & micro-interactions', 'Design systems & component libraries'], icon: <svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.2}><path d="M12 2L2 7l10 5 10-5-10-5z"/><path d="M2 17l10 5 10-5"/><path d="M2 12l10 5 10-5"/></svg> },
  { cat: 'Discipline 02', title: 'Development Architecture', desc: 'Every line of code serves a purpose. Modern frameworks, clean architecture, and a relentless commitment to technical performance at every layer.', list: ['React, Next.js & modern frameworks', 'Headless CMS architecture', 'Custom API integrations', 'Progressive web applications', 'Performance auditing & optimisation'], icon: <svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.2}><polyline points="16,18 22,12 16,6"/><polyline points="8,6 2,12 8,18"/></svg> },
  ...
  { cat: 'Discipline 04', title: 'Intelligent Automation', desc: 'Remove the manual. Amplify the meaningful. Smart automation that frees your team while delivering better experiences at scale.', list: ['CRM & ERP integrations', 'Behaviour-triggered email flows', 'Workflow automation (Make / Zapier)', 'Personalised dynamic content', 'AI-powered site features'], icon: <svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.2}><path d="M12 2a10 10 0 1 0 10 10"/><path d="M22 2L12 12"/><path d="M16 2h6v6"/></svg> },
  ...
  return (
    <>
      <style>{`
        ...
        .wd-a5{animation:wd-up .9s 1.1s ease forwards;opacity:0}

        .wd-ecocard { background:#111; border:1px solid rgba(255,255,255,.06); padding:36px 32px; transition:background .3s; }
        .wd-ecocard:hover { background:#1a1a1a; }

        .wd-capcard { flex-shrink:0; width:300px; background:#111; border:1px solid rgba(255,255,255,.06); padding:40px 32px; }

        .wd-procstep { background:#111; border:1px solid rgba(255,255,255,.06); padding:40px 28px; position:relative; transition:background .3s; }
        .wd-procstep:hover { background:#1a1a1a; }

        .wd-whyrow { padding:20px 0; border-bottom:1px solid rgba(255,255,255,.06); display:flex; gap:16px; }
        .wd-whyrow:last-child { border-bottom:none; }

        .wd-btn-white { background:#fff; color:#000; font-size:.58rem; font-weight:700; letter-spacing:.2em; text-transform:uppercase; padding:14px 28px; display:inline-block; transition:background .3s; }
        .wd-btn-white:hover { background:#e5e5e5; }
        .wd-btn-outline { border:1px solid rgba(255,255,255,.3); color:#fff; font-size:.58rem; letter-spacing:.2em; text-transform:uppercase; padding:14px 28px; display:inline-block; transition:all .3s; }
        .wd-btn-outline:hover { border-color:#fff; background:rgba(255,255,255,.08); }

        /* ── RESPONSIVE ── */
        .wd-hero { display:flex; flex-direction:column; min-height:100svh; }
        .wd-hero-left { padding:110px 24px 52px; background:linear-gradient(180deg,#000 60%,rgba(0,0,0,.8) 100%); }
        .wd-hero-right { min-height:280px; position:relative; overflow:hidden; }

        .wd-section { padding:4rem 24px; }
        .wd-2col { display:flex; flex-direction:column; gap:40px; }
        .wd-3grid { display:grid; grid-template-columns:1fr; gap:3px; }
        .wd-2grid { display:grid; grid-template-columns:1fr; gap:3px; }
        .wd-4strip { display:grid; grid-template-columns:1fr 1fr; height:160px; gap:3px; }

        .wd-badge { position:absolute; bottom:24px; right:16px; background:rgba(0,0,0,.85); backdrop-filter:blur(12px); border:1px solid rgba(255,255,255,.15); padding:18px 22px; width:180px; }

        .wd-manifesto-quote { font-family:var(--font-cormorant); font-size:clamp(1.9rem,7vw,4rem); font-weight:400; line-height:1.18; color:#fff; }
        ...
        .wd-why-img { min-height:320px; position:relative; overflow:hidden; }
        .wd-why-content { background:#111; border:1px solid rgba(255,255,255,.06); padding:44px 28px; }

        .wd-cta-section { padding:4rem 24px; text-align:center; background:#000; position:relative; overflow:hidden; }
        ...
        .wd-sig-section { position:relative; overflow:hidden; min-height:360px; display:flex; align-items:center; }
        .wd-sig-title { font-family:var(--font-cormorant); font-size:clamp(2rem,6vw,5.5rem); font-weight:400; line-height:1.1; color:#fff; margin-bottom:20px; }
        ...
        @media(min-width:768px){
          ...
          .wd-badge { width:220px; bottom:60px; right:20px; padding:24px 32px; }

          .wd-section { padding:6rem 3.75rem; }
          .wd-2col { flex-direction:row; align-items:center; gap:80px; }
          .wd-3grid { grid-template-columns:1fr 1fr 1fr; }
          .wd-2grid { grid-template-columns:1fr 1fr; }
          .wd-4strip { height:220px; grid-template-columns:repeat(4,1fr); }

          .wd-why-img { min-height:600px; }
          .wd-why-content { padding:64px 56px; }
          .wd-cta-section { padding:6rem 3.75rem; }
          .wd-sig-section { min-height:580px; }
      ...
      <main style={{ backgroundColor:'#000', color:'#fff', fontFamily:'var(--font-montserrat)' }}>
        ...
        <section className="wd-section" style={{ background:'#0a0a0a' }}>
          <div className="wd-2col" style={{ maxWidth:1280, margin:'0 auto' }}>
            <div style={{ flex:1 }}>
              <div style={{ fontFamily:'var(--font-cormorant)', fontSize:'7rem', lineHeight:.6, color:'rgba(255,255,255,.06)', marginBottom:-10 }}>&ldquo;</div>
            ...
            <div style={{ flex:1, display:'flex', flexDirection:'column', gap:32 }}>
              {[
                ...
                <div key={i} style={{ borderLeft:'1px solid rgba(255,255,255,.15)', paddingLeft:22 }}>
                  <div style={{ fontSize:'.72rem', fontWeight:600, letterSpacing:'.15em', textTransform:'uppercase', color:'#fff', marginBottom:10 }}>{item.title}</div>
        ...
        <section className="wd-section" style={{ background:'#000' }}>
          <div style={{ maxWidth:1280, margin:'0 auto' }}>
            <div style={{ marginBottom:48, display:'flex', flexDirection:'column', gap:16 }}>
              ...
              <p style={{ fontSize:'.78rem', lineHeight:2, color:'rgba(255,255,255,.5)', maxWidth:400 }}>Six interconnected disciplines. One unified architecture. Engineered to perform at the highest level.</p>
            </div>
            <div className="wd-3grid">
              <div className="wd-ecocard" style={{ padding:0, overflow:'hidden', position:'relative', minHeight:260 }}>
              ...
              {[
                ...
                <div key={i} className="wd-ecocard" style={{ display:'flex', flexDirection:'column' }}>
        ...
        {/* ══ CAPABILITIES ══ */}
        <section style={{ padding:'4rem 0 4rem 24px', background:'#0a0a0a', overflow:'hidden' }}>
          <div style={{ maxWidth:1280, margin:'0 0 40px', display:'flex', alignItems:'flex-end', justifyContent:'space-between', paddingRight:24 }}>
            <div>
          ...
          <div style={{ overflow:'hidden', position:'relative' }}>
            ...
            <div ref={trackRef} style={{ display:'flex', gap:3, transition:'transform .65s cubic-bezier(.16,1,.3,1)' }}>
              {capabilities.map((cap, i) => (
                <div key={i} className="wd-capcard">
                  ...
                  <ul style={{ listStyle:'none', display:'flex', flexDirection:'column', gap:8 }}>
                    {cap.list.map((li, j) => (
                      <li key={j} style={{ fontSize:'.7rem', color:'rgba(255,255,255,.5)', display:'flex', alignItems:'center', gap:10 }}>
                        <span style={{ width:16, height:1, background:'#fff', flexShrink:0, display:'inline-block' }}/>
                        {li}
                      </li>
                    ))}
                  </ul>
          ...
          <div style={{ display:'flex', gap:6, marginTop:28, paddingRight:24 }}>
            {capabilities.map((_, i) => (
              <button key={i} onClick={() => goTo(i)} style={{ height:2, width: i === capIdx ? 40 : 20, background: i === capIdx ? '#fff' : 'rgba(255,255,255,.2)', border:'none', cursor:'pointer', transition:'all .4s', padding:0 }}/>
            ))}
          </div>
        </section>
        ...
        <section className="wd-section" style={{ background:'#000' }}>
          <div style={{ maxWidth:1280, margin:'0 auto' }}>
            ...
            <h2 className="wd-section-title" style={{ marginBottom:40 }}>The Enhanccee <em style={{ fontStyle:'italic', color:'rgba(255,255,255,.7)' }}>Method</em></h2>
            <div className="wd-2grid">
              {processSteps.map((s, i) => (
                <div key={i} className="wd-procstep">
                  <div style={{ fontFamily:'var(--font-cormorant)', fontSize:'4rem', lineHeight:1, color:'rgba(255,255,255,.12)', position:'absolute', top:20, right:24 }}>{s.num}</div>
                  <div style={{ fontSize:'.55rem', letterSpacing:'.35em', textTransform:'uppercase', color:'rgba(255,255,255,.45)', marginBottom:10 }}>{s.phase}</div>
                  <h3 style={{ fontFamily:'var(--font-cormorant)', fontSize:'1.9rem', fontWeight:400, color:'#fff', marginBottom:14 }}>{s.title}</h3>
                  <p style={{ fontSize:'.77rem', lineHeight:2, color:'rgba(255,255,255,.5)' }}>{s.desc}</p>
              ...
              <div className="wd-procstep" style={{ gridColumn:'1 / -1' }}>
                <div style={{ fontFamily:'var(--font-cormorant)', fontSize:'4rem', lineHeight:1, color:'rgba(255,255,255,.12)', position:'absolute', top:20, right:24 }}>05</div>
                <div style={{ fontSize:'.55rem', letterSpacing:'.35em', textTransform:'uppercase', color:'rgba(255,255,255,.45)', marginBottom:10 }}>Phase Five</div>
                <h3 style={{ fontFamily:'var(--font-cormorant)', fontSize:'1.9rem', fontWeight:400, color:'#fff', marginBottom:14 }}>Launch &amp; Support</h3>
                <p style={{ fontSize:'.77rem', lineHeight:2, color:'rgba(255,255,255,.5)', maxWidth:680 }}>We don&apos;t deliver and disappear. Launch is the beginning of our partnership — with ongoing performance monitoring, continuous improvement, and strategic support.</p>
                <div style={{ display:'flex', flexWrap:'wrap', gap:8, marginTop:20 }}>
                  {['Staged Launch','Performance Monitoring','Ongoing Retainer','Growth Optimisation'].map((t,j) => (
                    <span key={j} style={{ padding:'5px 12px', border:'1px solid rgba(255,255,255,.12)', fontSize:'.57rem', letterSpacing:'.12em', color:'rgba(255,255,255,.5)', textTransform:'uppercase' }}>{t}</span>
                  ))}
                </div>
              </div>
            </div>
        ...
        {/* ══ WHY US ══ */}
        <section className="wd-section" style={{ background:'#0a0a0a' }}>
          <div className="wd-2grid" style={{ maxWidth:1280, margin:'0 auto' }}>
            ...
            <div className="wd-why-content">
              ...
              <p style={{ fontSize:'.8rem', lineHeight:2, color:'rgba(255,255,255,.5)', marginBottom:36 }}>We are not an agency. We are a digital atelier — a focused team of specialists who hold themselves to one standard: exceptional.</p>
              {whyRows.map((row, i) => (
                <div key={i} className="wd-whyrow">
                  ...
                  <div>
                    <div style={{ fontSize:'.73rem', fontWeight:600, letterSpacing:'.08em', color:'#fff', marginBottom:6, textTransform:'uppercase' }}>{row.title}</div>
        ...
        {/* ══ SIGNATURE ══ */}
        <section className="wd-sig-section">
          ...
          <div style={{ position:'relative', zIndex:2, width:'100%', textAlign:'center', padding:'60px 24px' }}>
            <p style={{ fontSize:'.58rem', letterSpacing:'.4em', textTransform:'uppercase', color:'rgba(255,255,255,.5)', marginBottom:28 }}>Enhanccee® — A Statement of Intent</p>
            <h2 className="wd-sig-title">
        ...
        <section className="wd-cta-section">
          ...
          <p style={{ fontSize:'.8rem', lineHeight:2, color:'rgba(255,255,255,.5)', marginBottom:40, maxWidth:560, margin:'0 auto 40px' }}>Select clients only. We partner with a limited number of brands each quarter.</p>
