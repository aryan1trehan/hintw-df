...
function initProgress(canvas: HTMLCanvasElement) {
  ...
  const logoImgs: HTMLImageElement[] = logoSrcs.map(src => {
    const img = new Image(); img.onload = () => { startTime = null }; img.src = src; return img
  })
  function draw(ts: number){
    if(!startTime)startTime=ts
    const loopT=((ts-startTime)/1000)%TOTAL_DUR
    ...
    logoSrcs.forEach((_,i)=>{
      ...
      ctx.save();ctx.globalAlpha=avA;ctx.beginPath();ctx.arc(avX,avY,avR,0,Math.PI*2);ctx.clip();ctx.fillStyle='#ffffff';ctx.fill()
      if(img&&img.complete&&img.naturalWidth>0){const p=avR*0.15;ctx.drawImage(img,avX-avR+p,avY-avR+p,(avR-p)*2,(avR-p)*2)}
      ctx.restore();ctx.save();ctx.globalAlpha=avA;ctx.beginPath();ctx.arc(avX,avY,avR,0,Math.PI*2);ctx.strokeStyle='rgba(110,232,202,0.5)';ctx.lineWidth=1.5*DPR;ctx.stroke();ctx.restore()
    })
